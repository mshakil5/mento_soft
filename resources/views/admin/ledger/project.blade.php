            ...
            <div class="col-md-10">
                <div class="card card-secondary">
                    <div class="card-header">
                        <h3 class="card-title">{{ $projectName }} Ledger</h3>
                    </div>
                    <div class="card-body">
                                ...
                                <tbody>
                                    ...
                                    @foreach($data as $txn)
                                        <tr>
                                            ...
                                            @if(in_array($txn->transaction_type, ['Due', 'Prepaid', 'Current', 'Payment', 'Purchase']))
                                                <td>{{ number_format($txn->at_amount, 0) }}</td>
                                                <td></td>
                                                @php $runningBalance -= $txn->at_amount; @endphp
                                            @elseif(in_array($txn->transaction_type, ['Received', 'Refund', 'Sold', 'Advance Adjust']))
                                                <td></td>
                                                <td>{{ number_format($txn->at_amount, 0) }}</td>
                                                @php $runningBalance += $txn->at_amount; @endphp
                                            @endif
